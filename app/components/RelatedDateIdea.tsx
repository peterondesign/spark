...
import { getPlaceholderImage } from "@/app/utils/imageService";
import { DateIdea } from "@/app/types/interfaces";
    ...
    const fetchDateIdea = async () => {
      try {
        const { data, error } = await supabase
          .from('date_ideas')
          .select('id, title, category, description, image, slug')
          .eq('slug', slug)
          .single();
        ...
        if (data) {
          setDateIdea(data as DateIdea);

          // Simplified image handling - just use the image URL directly or a placeholder
          setImageUrl(data.image || getPlaceholderImage(400, 200, data.title));
        }
      } catch (error) {
        console.error("Unexpected error fetching related date idea:", error);
      } finally {
        setLoading(false);
      }
    };
